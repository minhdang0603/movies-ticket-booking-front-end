...
import { FullscreenControl, GeolocateControl, Layer, LayerProps, Map, Marker, NavigationControl, Source } from 'react-map-gl';
import { CinemaResType } from '@/schemaValidations/cinema.schema';
...
export default function MapBox({ cinema }: { cinema: CinemaResType['data'] }) {
    const [start, setStart] = useState({ latitude: 0, longitude: 0 });
    const [end] = useState({ latitude: parseFloat(cinema.latitude), longitude: parseFloat(cinema.longitude) });
    ...
    const getRoute = async () => {
        ...
        const instruction = routeResponse.routes[0].legs[0].steps.map((step: any) => step.maneuver.instruction);
        setCoords(coords);
    ...
    const handleGeolocate = async (position: any) => {
    ...
    return (
        <div className='flex flex-col space-y-4'>
            <div style={{ width: '100%', height: '400px' }} className='relative overflow-hidden'>
                <Map
                    reuseMaps
            ...
            {routeInstructions.length > 0 && (
                <div className="mt-4 p-4 border-t">
                    <h2 className="text-lg font-semibold mb-2">Route Instructions</h2>
