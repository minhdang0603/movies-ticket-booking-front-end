...
export default function CinemaListHoverCard({
  cityList,
}: {
  ...
  useEffect(() => {
    dispatch({ type: "SET_CITIES", payload: cityList });
  }, [cityList, dispatch]);
  ...
  const fetchCinemas = async (cityId: string) => {
    if (!cinemas[cityId]) {
      const cinemaResponse = await cinemaApiRequest.getCinemasByCityAndMovie(
        cityId
      );
              ...
              <HoverCard>
                ...
                <HoverCardContent
                  ...
                  style={{
                    ...
                    overflowY:
                      cinemas[city.id]?.length > 6 ? "auto" : "visible",
                  }}
                >
                  {cinemas[city.id] ? (
                    <ScrollArea
                      className={
                        cinemas[city.id]?.length > 6 ? "h-full" : "h-auto"
                      }
                    >
                      {cinemas[city.id].map((cinema, index) => (
                        <Link
                          href={`/cinema/${cinema.cinemaId}`}
                          passHref
                          key={cinema.cinemaId}
                        >
                          {index > 0 && <Separator className="my-2" />}
