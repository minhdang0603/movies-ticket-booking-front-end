...
export default function MovieShowTime({ cityList, movieId }: {
        ...
        <div className='order-1 sm:order-2 sm:col-span-3 md:col-span-3 xl:col-span-6 lg:col-span-2 grid grid-cols-2 ml-5'>
          ...
          <div className='col-span-1'>
            ...
            >
              ...
              <SelectContent>
                <SelectItem value='0' className='cursor-pointer'>Tất cả rạp</SelectItem>
      ...
      <div>
        {
          shows.length > 0
            ? Object.values(showsByCinema).map((cinemaGroup, cinemaIndex) => (
              <div className='md:py-8 py-4 px-3 odd:bg-white even:bg-[#FDFBFA] even:border-t even:border-b' key={cinemaIndex}>
                <h1 className='text-base font-bold mb-4'>{cinemaGroup.cinema.name}</h1>
                {
                  Object.values(cinemaGroup.audioGroups).map((audioGroup, audioIndex) => (
                    <div className='flex md:flex-row flex-col gap-2 items-start mb-6' key={audioIndex}>
                      <label className='text-sm font-semibold text-grey-10 mt-2 w-[150px]'>{audioGroup.audio.type}</label>
