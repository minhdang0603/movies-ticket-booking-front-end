...
import { getAllCities, getCinemasByCityAndMovie, getShows, searchAddress } from '@/actions';
...
export default function CinemaInfo({ cinema }: {
    ...
    const toggleShowtimes = useCallback((movieId: string) => {
        setActiveMovieId(prevMovieId => prevMovieId === movieId ? null : movieId);
    }, [activeMovieId]);
                ...
                <div className='col-span-1 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-2 lg:flex-row gap-4 ml-2 flex-1'>
                    ...
                    <div className='col-span-1'>
                        <Select
                            ...
                            onValueChange={(value) => router.push(`/cinema/${value}`, { scroll: false })}
                        >
                            <SelectTrigger className="w-[180px] cursor-pointer">
                                <SelectValue />
                            </SelectTrigger>
                ...
                <div className='my-0 mx-auto items-center lg:max-w-screen-xl md:max-w-screen-lg md:px-4 sm:px-[45px] px-[16px] py-6'>
                    <div className='bg-white p-4'>
                        <div className='mb-4'>
                            ...
                            <div className='text-xl inline-block uppercase font-bold m-0'>PHIM</div>
                        </div>
                        <div>
                            <div className='overflow-x-scroll mt-7'>
                                <div className='flex justify-center space-x-5 capitalize text-center text-[14px] '>
                                    {days.map((day, index) => (
                                        ...
                                        >
                                            <Card className={`text-center ${selectedDate?.toDateString() === day.toDateString() ? 'bg-[#034ea2] text-white' : ''} border-none shadow-none`}>
                            ...
                            {shows.length > 0
                                ? (<ul className='grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 relative md:gap-8 gap-2'>
                                    {
                                        Object.values(showsByMovie).map((movieGroup, movieIndex) => (
                                            <li className='transition-all duration-300 ease-in-out' key={movieIndex}>
                                                <div className='relative text-center mb-2' onClick={() => toggleShowtimes(movieGroup.movie.id)}>
                                                    <div className='relative cursor-pointer'>
                                                        ...
                                                        <span className='text-sm'>{normalizeTitle(movieGroup.movie.title)}</span>
                                                    </div>
                                                    {activeMovieId === movieGroup.movie.id && (
                                                        <div className='absolute left-1/2 transform -translate-x-1/2 bottom-[-12px]'>
                                                            <svg width="24" height="24" fill="#034ea2" xmlns="http://www.w3.org/2000/svg">
                                                    ...
                                                    <>
                                                        <div className='absolute w-full left-0 z-10'>
                                                            <div className='bg-gray-50 border rounded px-4 py-6'>
                                                                <h4 className='text-base not-italic font-bold mb-4'>Suất chiếu</h4>
                                                                <div>
                                                                    {Object.values(movieGroup.audioGroups).map((audioGroup, audioIndex) => (
                                                                        <div
                                                                            className='grid md:grid-cols-8 grid-cols-1 grid-rows-5 md:grid-flow-row grid-flow-col items-center space-x-3'
                                                                            key={audioIndex}
                                                                        >
                                                                            <div className='md:col-span-2 md:row-span-5 xl:col-span-1 text-sm text-black-20'>{audioGroup.audio.type}</div>
                                ...
                                : (
                                    <div className='pt-20 pb-10 py-5 text-center'>
                                        <Image
                                            src={'https://www.galaxycine.vn/_next/static/media/video-camera.3c9f371d.png'}
                                            alt='Camera'
                                            className='inline object-cover duration-500 ease-in-out group-hover:opacity-100 scale-100 blur-0 grayscale-0)'
                                            width={80}
                                            height={80}
                                        />
                                        <p className='text-sm text-[#777777] mt-3'>Hiện tại phim chưa có lịch chiếu</p>
                                    </div>
                                )
                            }
                        </div>
                    ...
                    <div className='mt-8'>
                        <div className='bg-white p-4'>
                            <div className='mb-4'>
                                <span className='border-l-4 border-solid border-[#034ea2] mr-2'></span>
                                <span className='text-xl inline-block uppercase font-bold m-0'>Thông tin chi tiết</span>
                            </div>
                            <div>
                                <ul>
                                    <li>
